...
```{r echo = FALSE}
pacman::p_load(tidyverse, scales, dplyr, corrr, janitor, tidyr, psych)
```
...
```{r}
combined_data %>% 
  select(TARGET, n_loans, avg_loan_amt, max_loan_duration) %>% 
  drop_na() %>% 
  filter(max_loan_duration != '-Inf') %>% 
  pairs.panels()
```
There is no correlation between the target variable and the number of loans. low correlation for the other variables as well.

# Results Section

Home Credit has provided a lot of data and a vast amount of potential predictors to build this model. In my exploration I found some intuitively useless information such as the many columns regarding where the applicant currently resides. When I looked at this data further I found that over half of the clients provided actually had these fields missing. I think we should remove these columns from our analysis.

The data is also very heavily skewed toward the female gender, so we will need to account for that in our analysis and model building. 

I did not find many strong correlations between target and potential predictors, which makes it challenging to determine which variables we should use when building the model. The variables I am inclined to use are days_birth, days_employed, flag_own_car, potentially an interaction between loan_duration and number of loans, and education.
